...
class Lag_plot(object):
	...
	def plot_lightcurve(self,sigma = 1,fig = None):
		...
		for index_,(ratei,bsi,sigmai) in enumerate(rate_list):
			...
			if wind is not None:
				...
				ax.set_xlim(wind[0]-20,wind[-1]+20)
				ax.vlines(wind[0],0,ratei.max()+100,color='red',linestyle='dashed')
				ax.vlines(wind[-1],0,ratei.max()+100,color='red',linestyle='dashed')
				##
	...
	def plot_lag(self,ax = None):
		...
		energyc_err=(band_h-band_l)/2
		print('energyc',energyc,energyc_err)
		if ax is None:
			fig = plt.figure(constrained_layout=True)
			ax = fig.add_subplot(1,1,1)
		ax.errorbar(energyc[1:],lag[1:],xerr=energyc_err[1:],yerr = [lag_errl[1:],lag_errh[1:]],elinewidth=1,capsize=2,label = 'lag data',fmt = '.')
		
		ax.scatter(energyc[0],0)
		if ax is None:
			return fig
		else:
			return ax
